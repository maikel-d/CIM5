...
from ..models import Bien, DocumentoBien, CarpetaBien
...
class BienListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Carpetas de Bienes
        carpetas = []
        for carpeta in CarpetaBien.objects.all().order_by('orden', 'nombre'):
            bienes_carpeta = Bien.objects.filter(activo=True, carpeta=carpeta)
            carpetas.append({
                'id': carpeta.pk,
                'nombre': carpeta.nombre,
                'count': bienes_carpeta.count(),
                'bienes': bienes_carpeta,
            })
        context['carpetas'] = carpetas
        # Agrupar bienes por caso para la vista de carpetas
        todos_bienes = self.get_queryset()
        bienes_sin_caso = todos_bienes.filter(caso__isnull=True, carpeta__isnull=True)
        casos_con_bienes = []
...
def carpeta_bien_renombrar(request, pk):
    ...
    if request.method == "POST":
        ...
        if nombre:
            ...
            auditar(request, "ACTUALIZAR", "CarpetaBien", pk, nombre, "Bienes")
        else:
            messages.error(request, "Debe ingresar un nombre para la carpeta.")
    return redirect("bien_list")


@permiso_required(perms.BIENES_ELIMINAR)
def carpeta_bien_eliminar(request, pk):
    carpeta = get_object_or_404(CarpetaBien, pk=pk)
    nombre = carpeta.nombre
    Bien.objects.filter(carpeta=carpeta).update(carpeta=None)
    carpeta.delete()
    messages.success(request, f"Carpeta '{nombre}' eliminada.")
    auditar(request, "ELIMINAR", "CarpetaBien", pk, nombre, "Bienes")
    return redirect("bien_list")
